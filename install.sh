#!/usr/bin/env bash
# sni-router installer: downloads the latest release binary and sets up systemd.
#
# Usage:
...
#
# The service is installed but NOT started: edit the config and pick bind
# addresses that do not clash with services already running on the machine.
set -euo pipefail
...
mkdir -p "$CONF_DIR"
if [ ! -f "$CONF_DIR/sni-router.yaml" ]; then
